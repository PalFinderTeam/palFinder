...
class CachedProfileService @Inject constructor(
    ...
    private var cache = CachedRepository("profile", ProfileUser::class.java, db, time, contextProvider)
    ...
    override fun fetchAll(currentDate: Calendar?): Flow<List<ProfileUser>> = cache.fetchAll(currentDate)
    ...
    override suspend fun followUser(user: ProfileUser, targetId: String): Response<Unit> = db.followUser(user, targetId)

    override suspend fun unfollowUser(user: ProfileUser, targetId: String): Response<Unit> = db.unfollowUser(user, targetId)
}